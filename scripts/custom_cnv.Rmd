...
plot(density(gcNorm$TIL01), main ="TIL01 read-depth",
     xlab="normalized coverage per kb",cex.lab=1.6, xlim=c(0,25),col=alpha("red",0.9), lwd= 3)
lines(density(down_sw$TIL01),lwd= 3,xpd=FALSE,col ="blue")
legend("topright",c("CGH Chip","read-depth"),col=c("blue","red"), pch = 15, bty ="n", cex=1.3)
```
...
for ( s in samples ) {
  ...
  abline(v=down$B73.bam, col=alpha("red",0.1), lwd=0.1, lty="solid", lend=1)
  abline(v=up$B73.bam, col=alpha("blue",0.1), lwd=0.1, lty="solid", lend=1)
...
m1m2plot<-function(dt,chr,cols=rainbow(n=10),lim=-log(pnorm(q=lims[5],mean=0,sd=stddevs[5],lower.tail=FALSE)), m1, m2, cnvs) {
  ...
  for ( i in 1:length(chrs) ) {
    points(dt$pos[dt$chr==i & rownames(dt) %in% m1], dt$qval[dt$chr==i  & rownames(dt) %in% m1], xaxt="n", col="blue",cex=.7,pch=16)
    points(dt$pos[dt$chr==i & rownames(dt) %in% m2], dt$qval[dt$chr==i & rownames(dt) %in% m2], xaxt="n", col="red",cex=.7,pch=16)
...
#(length(cnvMaize1)/dim(cnvdf)[1])*100
length(cnvMaize1)/(length(na.omit(subGenomes[,2])))* 100
length(sw$GeneID[sw$GeneID %in% rownames(cnv.mat)])
#(length(cnvMaize2)/dim(cnvdf)[1])*100
length(cnvMaize2)/(length(na.omit(subGenomes[,3]))) * 100
```
...
```{r}
length(rownames(cnv.mat2)[rownames(cnv.mat2) %in% geneStats$Group.1[subgenome == "m1"]])/length(geneStats$Group.1[subgenome == "m1"]) )
length(rownames(cnv.mat2)[rownames(cnv.mat2) %in% geneStats$Group.1[subgenome == "m2"]])/length(geneStats$Group.1[subgenome == "m2"])
