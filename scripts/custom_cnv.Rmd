...
So what is the site frequency spectrum for CNVs (not SNPs within CNV regions but the CNV themselves).

```{r}
sfs<-apply(cnv.mat,1,function(x) table(x)[1])
#remove 0, as there is
hist(sfs,xlim=c(1,21), breaks = 20, col = "grey",main="Site Frequency Spectrum of down CNV")
```
...
clen.df<-read.table("~/chr_len.txt",header=FALSE)
# source some of Jinliang's scrip to draw a nice manhattan style plot
url<-"https://raw.githubusercontent.com/XLEvolutionist/custom_cnv/master/scripts/qmht.R"
data <- getURL(url,
               ssl.verifypeer=0L, followlocation=1L)
writeLines(data,'~/qmht.R')
source("~/qmht.R")
#quickMHTplot()

# now make a data.frame to send to the function.
#res<-data.frame()
