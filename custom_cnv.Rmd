...
We have the option of correcting for this using loess smoothing. First we need to melt the data:
```{r}
meltedNorm<-melt(subset(norm.df,select=-c(1:4,27,28)))
meltedNorm<-cbind(meltedNorm,"gc"=rep(norm.df$pct_gc,22))
print(head(meltedNorm))
```

Now produce the smoothed data with the `loess()` function of R. After some online research I found an R module called [`EDASeq`](http://bioconductor.org/packages/release/bioc/html/EDASeq.html) whixh can use loess smoothing to normalize counts within a library. You can use the `withinLaneNormalization` method to adjust count data according to GC content. Firstly, I must turn the coverage data set into a 
...
gcNorm<-withinLaneNormalization(x=countData, y=norm.df$pct_gc, which="loess")
head(gcNorm)
```